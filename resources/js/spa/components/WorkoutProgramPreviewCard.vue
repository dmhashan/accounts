    ...
    <div class="border-b border-slate-200 px-6 py-6 md:px-10">
      <div class="grid gap-4 md:grid-cols-[1.2fr_0.8fr] md:items-end">
        <div>
          ...
          <p v-if="program.creator_name" class="mt-2 text-sm text-slate-600">
            Prepared by {{ program.creator_name }}
          </p>
        </div>
          ...
          <table class="w-full border-collapse text-sm">
            ...
            <tbody>
              ...
              <tr v-if="!day.exercises || day.exercises.length === 0">
                <td colspan="7" class="border border-slate-300 px-3 py-3 text-center text-slate-500">
                  No exercises added.
                </td>
              </tr>
            </tbody>
          ...
          <table class="w-full border-collapse text-sm">
            ...
            <tbody>
              <tr v-for="(item, i) in cardioExtras" :key="item.id || i">
                <td class="border border-slate-300 px-3 py-2 align-top">
                  {{ formatFrequency(item.frequency_per_week) }}
                </td>
                <td class="border border-slate-300 px-3 py-2 align-top">
                  {{ item.cardio_type || '-' }}
                </td>
                <td class="border border-slate-300 px-3 py-2 align-top">
                  {{ item.duration_minutes ? `${item.duration_minutes} minute` : '-' }}
                </td>
                <td class="border border-slate-300 px-3 py-2 align-top">
                  {{ item.notes || '-' }}
                </td>
