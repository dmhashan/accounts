    ...
    <div class="fixed inset-0 z-40 flex items-center justify-center p-4" @keydown.escape.window="$emit('close')">
        ...
        <div class="relative z-10 w-full max-w-md rounded-2xl app-surface shadow-xl">
            ...
            <div class="flex items-start justify-between gap-3 px-5 py-4 border-b border-secondary-200 dark:border-secondary-700">
                <div>
                    <h3 class="text-base font-semibold text-secondary-900 dark:text-white">Add Temporary Member</h3>
                    <p class="text-xs text-secondary-500 dark:text-secondary-400 mt-0.5">Quick entry — only a name is required.</p>
                </div>
                <button
                    type="button"
            ...
            <form class="px-5 py-4 space-y-4" @submit.prevent="submit">
                ...
                <p v-if="errors.first_name" class="text-xs text-red-600 dark:text-red-400 -mt-2">{{ errors.first_name }}</p>

                <!-- Phone -->
                <AppFormField label="Phone Number" :optional="true">
                    <AppFormInput
                        ...
                        autocomplete="off"
                    />
                    <p v-if="errors.phone_number" class="mt-1 text-xs text-red-600 dark:text-red-400">{{ errors.phone_number }}</p>
                </AppFormField>
