<template>
  <div>
    ...
    <div class="flex items-center justify-between pt-12 pb-6">
      <div class="min-w-0">
        <p class="text-sm text-gray-400 leading-none mb-1">
          {{ greeting }},
        </p>
    ...
    >
      <div class="relative px-6 py-5 flex items-center justify-between overflow-hidden" style="background: linear-gradient(135deg, #059669 0%, #047857 60%, #065f46 100%);">
        <div class="absolute -top-8 -right-8 w-36 h-36 rounded-full bg-white/10 pointer-events-none" />
        <div class="absolute -bottom-6 -left-6 w-24 h-24 rounded-full bg-white/10 pointer-events-none" />
        <div class="relative">
          <p class="text-[11px] font-bold text-emerald-200 uppercase tracking-widest mb-0.5">
            Wallet Balance
          </p>
          <p class="text-3xl font-extrabold text-white tracking-tight leading-none">
        ...
        <div class="relative flex items-center gap-2">
          <div class="w-9 h-9 rounded-2xl bg-white/15 border border-white/25 flex items-center justify-center">
            <ChevronRight class="w-5 h-5 text-white" :stroke-width="2" />
          </div>
        </div>
      </div>
    </button>
    ...
    <section v-if="workoutsData.length" class="mb-5">
      ...
      >
        <div class="relative px-6 pt-6 pb-5 overflow-hidden" style="background:#1a1a1a; min-height:160px;">
          <div class="absolute -bottom-10 -right-10 w-44 h-44 rounded-full opacity-20 bg-black pointer-events-none" />
          <div class="absolute -top-6 -left-6 w-28 h-28 rounded-full opacity-10 bg-black pointer-events-none" />
          <div class="flex items-start justify-between">
            <div>
              <p class="text-xs font-bold text-red-500 uppercase tracking-widest mb-2">
                Active Plan
              </p>
              <h3 class="text-xl font-bold text-white leading-tight max-w-[200px]">
                {{ workoutsData[0].title }}
              </h3>
              <p v-if="workoutsData[0].creator_name" class="text-xs text-gray-400 mt-1">
                by {{ workoutsData[0].creator_name }}
              </p>
            </div>
          ...
          <div class="flex items-center gap-5 mt-5">
            <div>
              <p class="text-[10px] text-gray-400 mb-0.5 uppercase tracking-wider">
                Duration
              </p>
            ...
            <div>
              ...
              <p class="text-sm font-bold text-white">
                {{ workoutsData[0].days?.length || '-' }}
              </p>
            </div>
            <div class="ml-auto">
              <ChevronRight class="w-4 h-4 text-gray-400" :stroke-width="2" />
            </div>
          </div>
        </div>
      </button>
    ...
    <section v-if="salesData.length" class="mb-5">
      ...
      <div v-if="outstandingSales.length" class="mb-3 rounded-3xl overflow-hidden" style="background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%);">
        <div class="relative px-6 py-6 flex flex-col items-center text-center overflow-hidden">
          <!-- decorative blobs -->
          <div class="absolute -top-8 -right-8 w-36 h-36 rounded-full bg-white/10 pointer-events-none" />
          <div class="absolute -bottom-8 -left-8 w-28 h-28 rounded-full bg-white/10 pointer-events-none" />

          <p class="text-xs font-semibold text-red-100 uppercase tracking-widest mb-2">
            Total outstanding
          </p>
          <p class="text-5xl font-extrabold text-white tracking-tight leading-none">
      ...
      <div class="bg-white rounded-3xl overflow-hidden divide-y divide-gray-50 shadow-sm border border-gray-100">
        ...
        >
          ...
          >
            <FileText style="width:18px;height:18px" :class="!sale.is_paid ? 'text-red-400' : 'text-gray-500'" :stroke-width="1.8" />
          </div>
    ...
    <div v-if="!workoutsData.length && !salesData.length" class="flex flex-col items-center justify-center py-20 gap-3 text-gray-300">
      <Inbox class="w-12 h-12" :stroke-width="1" />
...
import { ChevronRight, Zap, Calendar, FileText, Inbox } from 'lucide-vue-next';
