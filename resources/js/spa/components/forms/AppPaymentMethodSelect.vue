<template>
  <div ref="rootRef" class="relative w-full">
    <button
      ...
      type="button"
      :disabled="disabled"
      class="app-form-control group h-12 w-full rounded-2xl border px-4 text-sm shadow-[0_1px_2px_rgba(15,23,42,0.04)] outline-none transition-all flex items-center gap-3 hover:border-secondary-400 hover:shadow-[0_2px_6px_rgba(15,23,42,0.08)] focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 disabled:cursor-not-allowed disabled:opacity-60 dark:hover:border-secondary-600"
    ...
    >
      ...
      <span
        class="flex-1 text-left truncate font-medium"
      ...
      <span
        v-if="selectedValue === 'member_wallet'"
      ...
      <ChevronDown
        class="flex-shrink-0 w-4 h-4 text-secondary-400 transition-transform duration-200 dark:text-secondary-500"
        ...
        >
          <div v-if="memberId" class="flex-shrink-0">
            <button
              ...
              class="relative w-full flex items-center gap-3 px-4 py-3.5 text-sm text-left transition-all duration-150 hover:bg-secondary-50 dark:hover:bg-secondary-800/60 disabled:cursor-not-allowed disabled:opacity-50 border-l-2"
              :class="selectedValue === 'member_wallet'
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <p class="font-semibold text-secondary-800 dark:text-secondary-100 text-sm leading-tight">
                  Member Wallet
                </p>
                <p class="text-xs text-secondary-400 dark:text-secondary-500 mt-0.5 leading-tight">
                  Pay from member balance
                </p>
              </div>
              <span
                class="flex-shrink-0 inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-xl border"
                :class="walletLoading
            ...
            <div v-if="methods.length" class="flex items-center gap-3 px-4 py-1.5">
              <div class="flex-1 h-px bg-secondary-100 dark:bg-secondary-800" />
              <span class="text-[10px] font-semibold uppercase tracking-widest text-secondary-400 dark:text-secondary-600">Methods</span>
              <div class="flex-1 h-px bg-secondary-100 dark:bg-secondary-800" />
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto overscroll-contain" :class="memberId ? '' : 'py-1.5'">
            ...
            >
              ...
              <span class="min-w-0 flex-1">
                ...
                <span class="mt-0.5 block text-xs text-secondary-400 dark:text-secondary-500 truncate">
                  {{ method.account_name || 'No account' }}<template v-if="methodSummary(method)"> - {{ methodSummary(method) }}</template>
                </span>
              </span>
              <span
                v-if="selectedValue === method.id"
                class="mt-1 flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-primary-500 text-white shadow-sm"
              >
                <Check class="w-3 h-3" />
              </span>
            </button>

            <div v-if="!methods.length && !memberId" class="flex flex-col items-center justify-center gap-2 px-4 py-8">
...
const props = defineProps({
    ...
    placeholder: { type: String, default: 'Select payment method...' },
});
...
const selectedLabel = computed(() => {
    ...
    const method = props.methods.find((item) => item.id === props.modelValue);
...
function methodSummary(method) {
    const parts = [];
    if (method.deduction_type === 'fixed') parts.push(`Fee ${formatBalance(method.deduction_value)}`);
    if (method.deduction_type === 'percentage') parts.push(`Fee ${Number(method.deduction_value || 0)}%`);
    if (method.requires_reconciliation) parts.push('Reconcile');
    return parts.join(', ');
}
...
function computePanelStyle() {
    ...
    const maxHeight = Math.min(openBelow ? spaceBelow : spaceAbove, viewportHeight * 0.55);
    panelStyle.value = {
