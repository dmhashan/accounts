    ...
    <div class="bg-white dark:bg-secondary-900 rounded-2xl shadow-2xl shadow-slate-900/10 overflow-hidden w-full">
        ...
        <div class="border-b border-secondary-200 dark:border-secondary-700 px-6 py-5 md:px-8">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div>
                    <p class="text-xs text-secondary-500 dark:text-secondary-400">Invoice</p>
                    <h2 class="text-2xl md:text-3xl font-bold tracking-tight text-secondary-900 dark:text-white">#{{ sale.id }}</h2>
                    <p class="mt-1 text-sm text-secondary-500 dark:text-secondary-400">{{ sale.created_at }}</p>
                ...
                <div class="flex flex-col gap-2 items-start sm:items-end shrink-0">
                    ...
                    <div v-if="sale.payment_method" class="flex items-center gap-1.5 text-xs text-secondary-500 dark:text-secondary-400">
                        <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
        ...
        <div class="px-4 py-5 md:px-8 space-y-5">
                ...
                <table class="w-full border-collapse text-sm">
                    <thead>
                        <tr class="bg-secondary-50 dark:bg-secondary-800 text-secondary-600 dark:text-secondary-400">
                            <th class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 text-left font-semibold">Item</th>
                            <th class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 text-right font-semibold">Qty</th>
                            <th class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 text-right font-semibold whitespace-nowrap">Unit Price</th>
                            <th class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 text-right font-semibold">Subtotal</th>
                        </tr>
                    </thead>
                    <tbody class="text-secondary-800 dark:text-secondary-200">
                        <tr v-for="(item, i) in sale.items" :key="i" class="hover:bg-secondary-50 dark:hover:bg-secondary-800/40">
                            ...
                            <td class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 align-top text-right">{{ item.quantity }}</td>
                            <td class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 align-top text-right">{{ money(item.unit_price) }}</td>
                            <td class="border border-secondary-200 dark:border-secondary-700 px-3 py-2 align-top text-right font-medium">{{ money(item.subtotal) }}</td>
                        </tr>
                        <tr v-if="!sale.items || sale.items.length === 0">
                            <td colspan="4" class="border border-secondary-200 dark:border-secondary-700 px-3 py-4 text-center text-secondary-500">No items on record.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 text-center">
                <div class="bg-secondary-50 dark:bg-secondary-800 rounded-xl px-3 py-3">
                    <p class="text-xs text-secondary-500 dark:text-secondary-400 mb-0.5">Total</p>
                    <p class="font-bold text-secondary-900 dark:text-white">{{ money(sale.total_amount) }}</p>
                </div>
                <div class="bg-green-50 dark:bg-green-900/20 rounded-xl px-3 py-3">
                    <p class="text-xs text-green-600 dark:text-green-400 mb-0.5">Paid</p>
                    <p class="font-bold text-green-700 dark:text-green-400">{{ money(sale.paid_amount) }}</p>
                </div>
                <div
