              ...
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <template v-if="loading">
                    <div class="app-skeleton mt-2 h-8 w-36 rounded-lg" />
                  </template>
          ...
          <div v-if="stockSummary.can_view" class="flex min-h-0 flex-col gap-3 shrink-0">
            ...
            <div v-if="!loading" class="grid grid-cols-3 gap-3 shrink-0">
              <div class="rounded-xl p-3 text-center" style="background: var(--surface-muted); border: 1px solid color-mix(in srgb, var(--surface-border) 88%, transparent)">
                ...
                <p class="mt-0.5 text-lg font-bold" style="color: var(--text-strong)">
                  {{ formatNumber(stockSummary.tracked_variations) }}
                </p>
              </div>
              <div class="rounded-xl p-3 text-center" style="background: var(--surface-muted); border: 1px solid color-mix(in srgb, var(--surface-border) 88%, transparent)">
                <p class="text-[11px] font-medium" style="color: var(--text-muted)">
                ...
                <p class="mt-0.5 text-lg font-bold" style="color: var(--text-strong)">
                  {{ formatNumber(stockSummary.available_units) }}
                </p>
              </div>
              <div class="rounded-xl p-3 text-center" :class="stockSummary.low_stock_variations > 0 ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800' : ''" :style="stockSummary.low_stock_variations === 0 ? 'background: var(--surface-muted); border: 1px solid color-mix(in srgb, var(--surface-border) 88%, transparent)' : ''">
                <p class="text-[11px] font-medium" :class="stockSummary.low_stock_variations > 0 ? 'text-red-500 dark:text-red-400' : ''" :style="stockSummary.low_stock_variations === 0 ? 'color: var(--text-muted)' : ''">
                  Low Stock
                </p>
                <p class="mt-0.5 text-lg font-bold" :class="stockSummary.low_stock_variations > 0 ? 'text-red-600 dark:text-red-400' : ''" :style="stockSummary.low_stock_variations === 0 ? 'color: var(--text-strong)' : ''">
                  {{ formatNumber(stockSummary.low_stock_variations) }}
                </p>
              </div>
            </div>
