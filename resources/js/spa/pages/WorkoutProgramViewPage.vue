    ...
    <section class="app-page-frame">
        ...
        <div v-else class="app-page-scroll space-y-5">
            <div class="app-surface rounded-2xl p-4 md:p-6">
                <h1 class="text-xl font-bold text-secondary-900 dark:text-white mb-2">{{ program.title }}</h1>

                <div v-if="program.description" class="text-sm text-secondary-700 dark:text-secondary-300 mb-4 whitespace-pre-wrap">{{ program.description }}</div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div v-if="program.duration_weeks">
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Duration</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ program.duration_weeks }} weeks</p>
                    </div>
                    <div v-if="program.days">
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Training Days</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ program.days.length }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="program.days && program.days.length > 0" class="space-y-4">
                <div v-for="day in sortedDays" :key="day.id || day.day_number" class="app-surface rounded-2xl overflow-hidden">
                    ...
                    <div v-else class="px-4 py-3 text-sm text-secondary-400 dark:text-secondary-500">No exercises added.</div>
                </div>
            </div>
        </div>
    </section>
