<template>
    <section class="app-page-frame">
        <AppPageHeader>
            ...
            <template #extra-slot>
                <AppSearchField v-model="search" placeholder="Search notifications by name" :disabled="loading" @search="load(1)" />
            </template>
        </AppPageHeader>
        ...
        <div class="min-h-0 flex flex-1 flex-col">
            <div class="app-page-scroll">
                <div class="app-surface rounded-2xl overflow-hidden">
                    <div v-if="loading" class="p-6 text-sm text-secondary-500 dark:text-secondary-400">Loading notifications...</div>
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <p class="text-sm font-semibold text-secondary-900 dark:text-white truncate">{{ n.name }}</p>
                                            <span class="px-2 py-0.5 text-[11px] font-semibold rounded-full" :class="statusClass(n.status)">{{ capitalize(n.status) }}</span>
                                        </div>
                                        <p class="mt-1 text-xs text-secondary-500 dark:text-secondary-400 line-clamp-2">{{ n.message }}</p>
                        ...
                        <table class="hidden md:table w-full text-sm">
                            <thead class="border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-800/50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left font-semibold text-secondary-700 dark:text-secondary-300">Message</th>
...
async function remove(id) {
    if (!confirm('Delete this draft notification?')) return;
    try {
