<template>
    <section class="app-page-frame">
        ...
        <div v-else-if="voucher" class="app-page-scroll space-y-5">
            ...
            <div class="app-surface rounded-2xl p-4 md:p-6">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
                    <div class="min-w-0">
                        <h1 class="text-xl font-bold text-secondary-900 dark:text-white">{{ voucher.name }}</h1>
                        <p class="mt-1 font-mono text-xs text-secondary-400 dark:text-secondary-500 select-all break-all">{{ voucher.uuid }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Amount</p>
                        <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ formatMoney(voucher.amount) }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Valid From</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ voucher.valid_from || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Valid Until</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ voucher.valid_until || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Created By</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ voucher.created_by?.name || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Created At</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ formatDate(voucher.created_at) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="voucher.redemption" class="app-surface rounded-2xl p-4 md:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Member</p>
                        <RouterLink
                        ...
                        <p v-else class="font-medium text-secondary-800 dark:text-secondary-200">—</p>
                        <p v-if="voucher.redemption.member?.member_id" class="text-xs text-secondary-400 mt-0.5">
                            {{ voucher.redemption.member.member_id }}
                        </p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Processed By</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ voucher.redemption.redeemed_by?.name || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Redeemed At</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ formatDate(voucher.redemption.redeemed_at) }}</p>
                    </div>
                </div>

                <div v-if="voucher.redemption.notes" class="mt-4">
                    <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Notes</p>
                    <p class="text-sm text-secondary-800 dark:text-secondary-200 whitespace-pre-wrap">{{ voucher.redemption.notes }}</p>
                </div>
            </div>
        </div>
        ...
        <div v-if="editModalOpen" class="fixed inset-0 z-40 flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/45" @click="closeEditModal"></div>
            <div class="relative z-10 w-full max-w-md rounded-2xl border border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-900 p-5 shadow-xl">
                <div class="flex items-start justify-between gap-3 mb-4">
                    <h3 class="text-lg font-semibold text-secondary-900 dark:text-white">Edit Voucher</h3>
                    <button type="button" class="text-secondary-400 hover:text-secondary-700 dark:hover:text-secondary-200" @click="closeEditModal">✕</button>
                </div>

                <div v-if="formError" class="mb-3 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-3 py-2 text-sm text-red-700 dark:text-red-200">{{ formError }}</div>
                ...
                <form class="space-y-3" @submit.prevent="submitEdit">
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1">
                        <button type="button" class="px-4 py-2 text-sm rounded-lg border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-200 hover:bg-secondary-100 dark:hover:bg-secondary-800" @click="closeEditModal">Cancel</button>
