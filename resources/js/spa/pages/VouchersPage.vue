<template>
    <section class="app-page-frame">
        <AppPageHeader>
            <template #cta-slot>
                <AppHeaderAction v-if="canManage" :icon="Ticket" label="New Voucher" @click="openCreateModal" />
            </template>
            <template #extra-slot>
                <AppSearchField v-model="search" placeholder="Search vouchers…" :disabled="loading" @search="load(1)">
                    ...
                    <div v-if="loading" class="divide-y divide-secondary-200 dark:divide-secondary-700">
                        <div v-for="i in 5" :key="i" class="p-4 space-y-2">
                            <div class="flex items-center gap-3">
                                <div class="app-skeleton h-3.5 w-36 rounded"></div>
                                <div class="app-skeleton h-3.5 w-16 rounded-full"></div>
                            </div>
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2 mb-1">
                                            <p class="text-sm font-semibold text-secondary-900 dark:text-white">{{ v.name }}</p>
                                            <span class="px-2 py-0.5 text-[11px] font-semibold rounded-full" :class="statusClass(v.status)">{{ capitalize(v.status) }}</span>
                                        </div>
                                        <p class="text-xs font-mono text-secondary-400 dark:text-secondary-500 break-all mb-1">{{ v.uuid }}</p>
                            ...
                            <table class="w-full">
                                <thead class="app-table-head-sticky bg-secondary-50 dark:bg-background-dark border-b border-secondary-200 dark:border-secondary-700">
                                    <tr>
                                        ...
                                        <th class="app-table-th">UUID</th>
                                        <th class="app-table-th text-right">Amount</th>
                                        <th class="app-table-th">Status</th>
                                        <th class="app-table-th">Valid From</th>
                                        <th class="app-table-th">Valid Until</th>
                                        <th class="app-table-th">Created</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
                                    <tr
                                        ...
                                        @click="router.push('/vouchers/' + v.id)"
                                    >
                                        <td class="app-table-td font-semibold">{{ v.name }}</td>
                                        <td class="app-table-td">
                                            <span class="font-mono text-xs text-secondary-500 dark:text-secondary-400 select-all">{{ v.uuid }}</span>
                                        </td>
                                        <td class="app-table-td text-right font-semibold text-emerald-600 dark:text-emerald-400">{{ formatMoney(v.amount) }}</td>
                                        <td class="app-table-td">
                                            <span class="px-2 py-0.5 text-xs font-semibold rounded-full" :class="statusClass(v.status)">{{ capitalize(v.status) }}</span>
                                        </td>
                                        <td class="app-table-td text-secondary-500 dark:text-secondary-400">{{ v.valid_from || '—' }}</td>
                                        <td class="app-table-td text-secondary-500 dark:text-secondary-400">{{ v.valid_until || '—' }}</td>
                                        <td class="app-table-td text-secondary-500 dark:text-secondary-400 whitespace-nowrap">{{ formatDate(v.created_at) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
        ...
        <div v-if="formModalOpen" class="fixed inset-0 z-40 flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/45" @click="closeFormModal"></div>
            <div class="relative z-10 w-full max-w-md rounded-2xl border border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-900 p-5 shadow-xl">
                <div class="flex items-start justify-between gap-3 mb-4">
                    <h3 class="text-lg font-semibold text-secondary-900 dark:text-white">New Voucher</h3>
                    <button type="button" class="text-secondary-400 hover:text-secondary-700 dark:hover:text-secondary-200" @click="closeFormModal">✕</button>
                </div>
                ...
                <form class="space-y-3" @submit.prevent="submitForm">
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1">
                        <button type="button" class="px-4 py-2 text-sm rounded-lg border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-200 hover:bg-secondary-100 dark:hover:bg-secondary-800" @click="closeFormModal">Cancel</button>
