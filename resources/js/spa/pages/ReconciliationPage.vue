    ...
    <div class="app-page-scroll space-y-4">
      ...
      <template v-else>
        ...
        <div class="app-surface rounded-2xl p-5 md:p-6 space-y-4">
          ...
          <template v-else>
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
              <div>
                <p class="text-secondary-500 dark:text-secondary-400 text-xs">
                  Opened by
                </p>
        ...
        <div v-if="canManage" class="app-surface rounded-2xl overflow-hidden">
          ...
          <div v-if="historyLoading" class="p-6 text-sm text-secondary-500 dark:text-secondary-400 text-center">
            Loading…
          </div>
          ...
          <div v-else class="md:hidden divide-y divide-secondary-200 dark:divide-secondary-700">
            <article v-for="s in history" :key="s.id" class="p-4 space-y-1">
              ...
              <RouterLink :to="`/reconciliation/sessions/${s.id}`" class="text-xs text-primary-600 dark:text-primary-400">
                View →
              </RouterLink>
            </article>
            <div v-if="history.length === 0" class="p-6 text-sm text-secondary-500 dark:text-secondary-400">
              No sessions yet.
            </div>
          </div>
              ...
              <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
                <tr v-for="s in history" :key="s.id" class="hover:bg-secondary-50 dark:hover:bg-secondary-800/50">
                  ...
                  <td class="px-6 py-4 text-right text-sm">
                    <RouterLink :to="`/reconciliation/sessions/${s.id}`" class="text-primary-600 dark:text-primary-400 hover:underline">
                      View
                    </RouterLink>
                  </td>
                </tr>
          ...
          <div v-if="historyMeta.last_page > 1" class="app-page-pagination">
            <AppPagination
