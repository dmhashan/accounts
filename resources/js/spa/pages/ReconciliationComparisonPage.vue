    ...
    <div class="app-page-scroll space-y-4">
      ...
      <template v-else>
          ...
          <div class="md:hidden divide-y divide-secondary-200 dark:divide-secondary-700">
            <article v-for="item in items" :key="`${item.type}-${item.reference_id}`" class="p-4 space-y-2">
              ...
              <div class="grid grid-cols-3 gap-2 text-xs text-secondary-600 dark:text-secondary-400">
                <div>
                  <p class="text-secondary-400 dark:text-secondary-500">
                    Opening
                  </p>
            ...
            <div v-if="items.length === 0" class="p-6 text-sm text-secondary-500 dark:text-secondary-400">
              No items.
            </div>
          </div>
          ...
          <div class="hidden md:block overflow-x-auto">
            <table class="w-full">
              <thead class="bg-secondary-50 dark:bg-background-dark border-b border-secondary-200 dark:border-secondary-700">
                <tr>
                  <th class="px-5 py-3 text-left text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase">
                    Item
                  </th>
                  <th class="px-5 py-3 text-left text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase">
                    Type
                  </th>
                  <th class="px-5 py-3 text-right text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase">
                    Opening
                  </th>
                  <th class="px-5 py-3 text-right text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase">
                    System Changes
              ...
              <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
                <tr v-for="item in items" :key="`${item.type}-${item.reference_id}`" class="hover:bg-secondary-50 dark:hover:bg-secondary-800/50">
                  ...
                  <td class="px-5 py-4 text-sm text-secondary-500 dark:text-secondary-400">
                    {{ item.type }}
                  </td>
                  <td class="px-5 py-4 text-sm text-right text-secondary-700 dark:text-secondary-300">
                    {{ item.opening_value }}
                  </td>
                ...
                <tr v-if="items.length === 0">
                  <td colspan="7" class="px-5 py-10 text-center text-sm text-secondary-500 dark:text-secondary-400">
                    No items.
                  </td>
                </tr>
              </tbody>
        ...
        <div v-else-if="session?.adjustment_reason" class="app-surface rounded-2xl p-5 md:p-6">
          <p class="text-xs text-secondary-500 dark:text-secondary-400 mb-1 uppercase font-medium tracking-wide">
            Adjustment Reason
          </p>
