...
/** Normalised program prop for WorkoutProgramPreviewCard */
const previewProgram = computed(() => ({
    title: previewProgramTitle.value,
    duration_weeks: builderForm.value.duration_weeks,
    creator_name: currentUserFullName.value,
    days: previewDays.value.map((day) => ({
        day_number: day.day_number,
        title: day.title,
        exercises: orderedExerciseRows(day.exercises).map((row) => ({
            exercise_name: displayExerciseName(row),
    ...
    extras: [
        ...previewCoreExtras.value.map((e) => ({ ...e, type: 'core' })),
        ...previewCardioExtras.value.map((e) => ({ ...e, type: 'cardio' })),
    ],
}));
