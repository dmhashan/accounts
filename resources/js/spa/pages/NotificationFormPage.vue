<template>
    <section class="app-page-frame">
        <AppPageHeader :show-back="true" :title="isEdit ? 'Edit Notification' : 'New Notification'" />

        <div class="app-page-scroll">
            <div v-if="pageLoading" class="p-6 text-sm text-secondary-500 dark:text-secondary-400">Loading...</div>
            ...
            <template v-else>
                ...
                <form class="space-y-4" @submit.prevent="save">
                    ...
                    <div class="app-surface rounded-2xl p-4 md:p-6 space-y-4">
                        <h3 class="text-base font-semibold text-secondary-900 dark:text-white">Notification Details</h3>

                        <AppFormField label="Name" :required="true">
                            <AppFormInput v-model="form.name" type="text" maxlength="255" placeholder="e.g. Monthly membership reminder" required />
                        </AppFormField>

                        <AppFormField label="Message" :required="true">
                            <AppFormTextarea v-model="form.message" rows="5" maxlength="621" placeholder="Type your SMS message here..." required />
                            <p class="mt-1 text-xs text-secondary-400 dark:text-secondary-500 text-right">{{ form.message.length }} / 621</p>
                        </AppFormField>
                    </div>
                    ...
                    <div class="app-surface rounded-2xl p-4 md:p-6 space-y-4">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button type="button" class="px-3 py-1.5 text-xs font-medium rounded-lg border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-700 transition-colors" @click="selectAll">Select All</button>
                                <button type="button" class="px-3 py-1.5 text-xs font-medium rounded-lg border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-700 transition-colors" @click="selectActive">Active Only</button>
                                <button type="button" class="px-3 py-1.5 text-xs font-medium rounded-lg border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-700 transition-colors" @click="selectInactive">Inactive Only</button>
                                <button type="button" class="px-3 py-1.5 text-xs font-medium rounded-lg border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" @click="clearAll">Clear</button>
                            </div>
                        </div>
                        ...
                        <!-- List -->
                        <div v-if="membersLoading" class="py-6 text-center text-sm text-secondary-500 dark:text-secondary-400">Loading members...</div>

                        <div v-else-if="filteredMembers.length === 0" class="py-6 text-center text-sm text-secondary-500 dark:text-secondary-400">No members with phone numbers found.</div>
                            ...
                            <div class="max-h-80 overflow-y-auto divide-y divide-secondary-100 dark:divide-secondary-800">
                                ...
                                >
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-secondary-500 dark:text-secondary-400 truncate">{{ m.member_id }} &bull; {{ m.phone_number }}</p>
                                    </div>
                        ...
                        <p v-if="recipientError" class="text-xs text-red-600 dark:text-red-400">{{ recipientError }}</p>
                    </div>

                    <!-- Actions -->
                    <div class="flex flex-col sm:flex-row items-center justify-end gap-2 pb-4">
                        <RouterLink to="/notifications" class="w-full sm:w-auto text-center px-4 py-2 border border-secondary-300 dark:border-secondary-700 rounded-lg text-sm text-secondary-700 dark:text-secondary-300">Cancel</RouterLink>
