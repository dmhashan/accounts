        ...
        <div v-else class="app-page-scroll space-y-5">
            <div class="app-surface rounded-2xl p-4 md:p-6">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                    <h1 class="text-xl font-bold text-secondary-900 dark:text-white">{{ exercise.name }}</h1>
                    <span class="self-start rounded-full px-3 py-1 text-xs font-semibold"
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Default Sets</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ exercise.default_sets || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Default Reps</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ exercise.default_reps || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Tempo</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ exercise.default_tempo || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Rest (s)</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ exercise.default_rest ?? '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="exercise.variations && exercise.variations.length > 0" class="app-surface rounded-2xl overflow-hidden">
                <div class="px-4 md:px-6 py-4 border-b border-secondary-200 dark:border-secondary-700">
                    <h2 class="text-base font-semibold text-secondary-900 dark:text-white">Variations ({{ exercise.variations.length }})</h2>
                </div>
