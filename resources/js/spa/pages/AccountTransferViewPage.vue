        ...
        <div v-else class="app-page-scroll space-y-5">
            <div class="app-surface rounded-2xl p-4 md:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Date</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ transfer.transfer_date || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">From</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ transfer.source_account_name || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">To</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ transfer.destination_account_name || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Reference</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ transfer.reference_number || '—' }}</p>
                    </div>
                </div>

                <div v-if="transfer.notes" class="mt-4">
                    <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Notes</p>
                    <p class="text-sm text-secondary-800 dark:text-secondary-200 whitespace-pre-wrap">{{ transfer.notes }}</p>
                </div>
            </div>
        </div>
    </section>
