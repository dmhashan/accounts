    ...
    <section class="app-page-frame">
        ...
        <div v-else class="app-page-scroll space-y-5">
            <div class="app-surface rounded-2xl p-4 md:p-6">
                <h1 class="text-xl font-bold text-secondary-900 dark:text-white mb-1">{{ assignment.member_name }}</h1>
                <p v-if="assignment.member_code" class="text-sm text-secondary-500 dark:text-secondary-400 mb-4">#{{ assignment.member_code }}</p>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Assigned Program</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ assignment.assigned_program_title || assignment.program_title || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Effective Date</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ assignment.effective_date || '—' }}</p>
                    </div>
                    <div v-if="assignment.created_at">
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Assigned On</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ assignment.created_at }}</p>
                    </div>
                </div>
            </div>
        </div>
