    ...
    <section class="app-page-frame">
        ...
        <div class="mb-3 flex flex-wrap gap-2 shrink-0">
            ...
            >
                ...
                <option value="tab_view">Tab view</option>
                <option value="workout_opened">Workout opened</option>
                <option value="sale_opened">Sale opened</option>
        ...
        <div class="min-h-0 flex flex-1 flex-col">
            <div class="app-page-scroll">
                <div class="app-surface rounded-2xl overflow-hidden">
                    <div v-if="loading" class="p-6 text-sm text-secondary-500 dark:text-secondary-400">Loading activity logs...</div>
                        ...
                        <div class="md:hidden divide-y divide-secondary-200 dark:divide-secondary-700">
                            <article v-for="log in logs" :key="log.id" class="p-4 space-y-2">
                                ...
                                <div v-if="expandedId === log.id" class="rounded-lg bg-secondary-50 dark:bg-secondary-800/40 p-3 text-xs space-y-1 text-secondary-600 dark:text-secondary-400 break-all">
                                    <p v-if="log.screen_width"><strong>Screen:</strong> {{ log.screen_width }}×{{ log.screen_height }}</p>
                                    <p><strong>Session:</strong> {{ log.session_id }}</p>
                                    <p v-if="log.metadata"><strong>Metadata:</strong> {{ JSON.stringify(log.metadata) }}</p>
                                </div>
                            </article>
                        </div>
                        ...
                        <table class="hidden md:table w-full text-sm">
                            <thead class="border-b border-secondary-200 dark:border-secondary-700 bg-secondary-50 dark:bg-secondary-800/50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left font-semibold text-secondary-700 dark:text-secondary-300 hidden xl:table-cell">Screen</th>
                                </tr>
                            </thead>
