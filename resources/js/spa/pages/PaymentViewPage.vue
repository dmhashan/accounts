        ...
        <div v-else class="app-page-scroll space-y-5">
            <div class="app-surface rounded-2xl p-4 md:p-6">
                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-4">
                    <div>
                        <h1 class="text-xl font-bold text-secondary-900 dark:text-white">{{ payment.member_name || 'Walk-in / Unspecified' }}</h1>
                        <p v-if="payment.member_phone" class="mt-1 text-sm text-secondary-500 dark:text-secondary-400">{{ payment.member_phone }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Payment Date</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ payment.payment_date || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Account</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ payment.account_name || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Reference</p>
                        <p class="font-medium text-secondary-800 dark:text-secondary-200">{{ payment.reference_number || '—' }}</p>
                    </div>
                </div>

                <div v-if="payment.notes" class="mt-4">
                    <p class="text-xs text-secondary-400 uppercase tracking-wide mb-1">Notes</p>
                    <p class="text-sm text-secondary-800 dark:text-secondary-200 whitespace-pre-wrap">{{ payment.notes }}</p>
                </div>
            </div>
        </div>
    </section>
