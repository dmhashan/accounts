                ...
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-0.5 text-xs text-secondary-400 dark:text-secondary-500">
                      ...
                      <template v-if="ev.total_paid > 0">
                        &bull; <span class="text-green-600 dark:text-green-400">{{ formatFee(ev.total_paid) }} paid</span>
                      </template>
                      <template v-if="ev.total_outstanding > 0">
                        &bull; <span class="text-amber-600 dark:text-amber-400">{{ formatFee(ev.total_outstanding) }} outstanding</span>
                      </template>
                    </p>
                  </div>
            ...
            <table class="hidden md:table w-full text-sm">
              ...
              <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
                ...
                >
                  ...
                  <td class="px-4 py-3 text-right whitespace-nowrap font-medium" :class="ev.total_paid > 0 ? 'text-green-600 dark:text-green-400' : 'text-secondary-400 dark:text-secondary-500'">
                    {{ ev.total_paid > 0 ? formatFee(ev.total_paid) : '—' }}
                  </td>
                  <td class="px-4 py-3 text-right whitespace-nowrap font-medium" :class="ev.total_outstanding > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-secondary-400 dark:text-secondary-500'">
                    {{ ev.total_outstanding > 0 ? formatFee(ev.total_outstanding) : '—' }}
                  </td>
                  <td class="px-4 py-3">
