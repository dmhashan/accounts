<template>
    <section class="app-page-frame">
        <AppPageHeader>
            ...
            <template #extra-slot>
                <AppSearchField v-model="search" placeholder="Search forms…" :disabled="loading" @search="load" />
            </template>
        </AppPageHeader>
                ...
                <div class="app-surface rounded-2xl overflow-hidden">
                    ...
                    <div v-if="loading" class="divide-y divide-secondary-200 dark:divide-secondary-700">
                        <div v-for="i in 5" :key="i" class="p-4 space-y-2">
                            <div class="app-skeleton h-3.5 w-48 rounded"></div>
                            <div class="app-skeleton h-3 w-72 rounded"></div>
                        </div>
                    </div>
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2 mb-1">
                                            <p class="text-sm font-semibold text-secondary-900 dark:text-white truncate">{{ t.title }}</p>
                                        ...
                                        <p v-if="t.description" class="text-xs text-secondary-500 dark:text-secondary-400 line-clamp-2">{{ t.description }}</p>
                        ...
                        <div class="hidden md:block app-table-scroll">
                            <table class="w-full">
                                <thead class="app-table-head-sticky bg-secondary-50 dark:bg-background-dark border-b border-secondary-200 dark:border-secondary-700">
                                    <tr>
                                        <th class="app-table-th">Title</th>
                                        <th class="app-table-th">Description</th>
                                        <th class="app-table-th text-center">Fields</th>
                                        <th class="app-table-th text-center">Status</th>
                                        <th class="app-table-th">Created by</th>
                                ...
                                <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
                                    <tr
                                        ...
                                        @click="router.push('/forms/' + t.id + '/submissions')"
                                    >
                                        <td class="app-table-td font-semibold text-secondary-900 dark:text-white">{{ t.title }}</td>
                                        <td class="app-table-td text-secondary-500 dark:text-secondary-400 max-w-xs truncate">{{ t.description || '—' }}</td>
                                        <td class="app-table-td text-center">{{ t.fields_count }}</td>
                                        ...
                                        <td class="app-table-td text-secondary-500 dark:text-secondary-400">{{ t.created_by?.name || '—' }}</td>
                                        <td class="app-table-td text-secondary-500 dark:text-secondary-400 whitespace-nowrap">{{ t.created_at }}</td>
        ...
        <div v-if="deleteTarget" class="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 w-full max-w-sm rounded-2xl border border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-900 p-5 shadow-xl">
                <h3 class="text-base font-semibold text-secondary-900 dark:text-white mb-2">Delete Form Template?</h3>
