                ...
                <div class="app-surface rounded-2xl overflow-hidden">
                    ...
                    <template v-else>
                        ...
                        <div class="md:hidden divide-y divide-secondary-200 dark:divide-secondary-700">
                            <article v-for="payment in payments" :key="payment.id" class="p-4 space-y-1">
                                <div class="flex justify-between items-start gap-3">
                                    ...
                                    <div v-if="canManage" class="flex gap-2 shrink-0">
                                        <RouterLink :to="`/payments/${payment.id}/edit`" class="text-primary-600 dark:text-primary-400 text-sm">Edit</RouterLink>
                                        <button type="button" class="text-red-600 dark:text-red-400 text-sm" @click="removePayment(payment.id)">Delete</button>
                                    </div>
                                </div>
                            </article>
                            ...
                            <table class="w-full">
                                <thead class="app-table-head-sticky bg-secondary-50 dark:bg-background-dark border-b border-secondary-200 dark:border-secondary-700">
                                    <tr>
                                        ...
                                        <th v-if="canManage" class="px-6 py-3 text-right text-xs font-medium text-secondary-500 dark:text-secondary-400 uppercase">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700">
                                    <tr v-for="payment in payments" :key="payment.id" class="hover:bg-secondary-50 dark:hover:bg-secondary-800/50">
                                        <td class="px-6 py-4">
                                        ...
                                        <td v-if="canManage" class="px-6 py-4 text-right whitespace-nowrap space-x-2">
                                            <RouterLink :to="`/payments/${payment.id}/edit`" class="text-primary-600 dark:text-primary-400 text-sm hover:underline">Edit</RouterLink>
                                            <button type="button" class="text-red-600 dark:text-red-400 text-sm hover:underline" @click="removePayment(payment.id)">Delete</button>
                                        </td>
                                    </tr>
                                    <tr v-if="payments.length === 0">
                                        <td :colspan="canManage ? 7 : 6" class="px-6 py-8 text-center text-sm text-secondary-500 dark:text-secondary-400">No payments recorded.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
