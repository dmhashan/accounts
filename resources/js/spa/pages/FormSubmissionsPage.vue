<template>
  <section class="app-page-frame">
    <AppPageHeader show-back :title="templateTitle ? templateTitle + ' — Submissions' : 'Form Submissions'">
      <template #cta-slot>
        <button
          type="button"
                    ...
                    <td class="app-table-td">
                      <div class="flex items-center justify-end gap-1">
                        <button
                          v-if="s.has_pdf"
                          type="button"
                          class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg border border-secondary-200 dark:border-secondary-700 text-secondary-600 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-800 transition-colors"
                          @click="openPdf(s)"
                        >
                          <FileDown class="w-3.5 h-3.5" :stroke-width="1.75" />
                          PDF
                        </button>
                        <button type="button" class="px-2.5 py-1 text-xs font-semibold rounded-lg border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" @click="confirmDelete(s)">
                          Delete
                        </button>
                      </div>
                    </td>
    ...
    <div v-if="fillModalOpen" class="fixed inset-0 z-40 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black/45" @click="closeFillModal" />
      <div class="relative z-10 w-full max-w-xl rounded-2xl border border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-900 shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div class="flex items-start justify-between gap-3 px-5 py-4 border-b border-secondary-100 dark:border-secondary-800 shrink-0">
          <div>
            ...
            <p v-if="fillMember" class="text-xs text-secondary-500 dark:text-secondary-400 mt-0.5">
              Filling for: <strong>{{ fillMember.label }}</strong>
            </p>
          </div>
          <button type="button" class="text-secondary-400 hover:text-secondary-700 dark:hover:text-secondary-200 mt-0.5" @click="closeFillModal">
            ✕
          </button>
        </div>
        ...
        <div class="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          <div v-if="!fillMember" class="space-y-3">
            <p class="text-sm text-secondary-700 dark:text-secondary-300">
              Search and select a member to fill this form for:
            </p>
            <div class="relative">
              <input
            ...
            <div v-if="memberResults.length > 0" class="rounded-xl border border-secondary-200 dark:border-secondary-700 overflow-hidden">
              ...
              >
                ...
                <div class="min-w-0">
                  <p class="font-semibold text-secondary-900 dark:text-white truncate">
                    {{ m.label }}
                  </p>
                  <p class="text-[11px] text-secondary-400 dark:text-secondary-500">
                    {{ m.member_id }}
                  </p>
                </div>
              </button>
            </div>
            <p v-else-if="memberSearch.trim().length >= 2 && !memberSearching" class="text-xs text-secondary-500 dark:text-secondary-400">
              No members found.
            </p>
          </div>
          ...
          <template v-else>
            ...
            <div v-if="fillError" class="rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-3 py-2 text-sm text-red-700 dark:text-red-200">
              {{ fillError }}
            </div>
        ...
        <div v-if="fillMember" class="px-5 py-4 border-t border-secondary-100 dark:border-secondary-800 flex items-center justify-between gap-3 shrink-0">
          <button type="button" class="text-sm text-secondary-500 hover:text-secondary-700 dark:hover:text-secondary-300" @click="fillMember = null; fillResponses = {}">
            ← Change member
          </button>
          <div class="flex items-center gap-2">
            <button type="button" class="px-4 py-2 text-sm rounded-lg border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-200 hover:bg-secondary-100 dark:hover:bg-secondary-800" @click="closeFillModal">
              Cancel
            </button>
            <button
              type="button"
              ...
              @click="submitFill"
            >
              {{ fillSubmitting ? 'Submitting…' : 'Submit & Generate PDF' }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="deleteTarget" class="fixed inset-0 z-40 flex items-center justify-center p-4">
      ...
      <div class="relative z-10 w-full max-w-sm rounded-2xl border border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-900 p-5 shadow-xl">
        ...
        <p class="text-sm text-secondary-500 dark:text-secondary-400 mb-4">
          This submission and its PDF will be permanently deleted.
        </p>
