<template>
        ...
        <div class="app-surface rounded-2xl p-4 md:p-6 mb-4">
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div class="space-y-1 min-w-0">
              ...
              <p class="text-base font-semibold text-secondary-900 dark:text-white">
                {{ notification.name }}
              </p>
            </div>
          ...
          <div class="mt-4 space-y-1">
            ...
            <div class="rounded-xl bg-secondary-50 dark:bg-secondary-800/50 px-4 py-3 text-sm text-secondary-800 dark:text-secondary-200 whitespace-pre-wrap leading-relaxed">
            ...
            <table class="hidden md:table w-full text-sm">
              ...
              <tbody class="divide-y divide-secondary-100 dark:divide-secondary-800">
                <tr v-for="(r, i) in notification.recipients" :key="r.id" class="hover:bg-secondary-50 dark:hover:bg-secondary-800/30">
                  <td class="px-4 py-3 text-secondary-500 dark:text-secondary-400">
                    {{ i + 1 }}
                  </td>
    ...
    <div v-if="showConfirm" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-4">
      <div class="w-full sm:max-w-md app-surface rounded-2xl p-6 space-y-4 shadow-xl">
        ...
        <div class="flex gap-2 justify-end">
          <button type="button" class="px-4 py-2 border border-secondary-300 dark:border-secondary-700 rounded-lg text-sm text-secondary-700 dark:text-secondary-300" @click="showConfirm = false">
            Cancel
          </button>
          <button
            type="button"
            class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-semibold disabled:opacity-50 flex items-center gap-2"
            :disabled="sending"
            @click="doSend"
          >
            <Send class="h-4 w-4" />
  ...
  <AppConfirmModal
    v-if="deleteNotificationConfirm"
    title="Delete Notification"
    message="Delete this draft notification? This cannot be undone."
    confirm-label="Delete"
    loading-label="Deleting..."
    :loading="deleting"
    @confirm="confirmRemoveNotification"
    @cancel="deleteNotificationConfirm = false"
  />
</template>
...
const deleteNotificationConfirm = ref(false);

function removeNotification() {
    deleteNotificationConfirm.value = true;
}

async function confirmRemoveNotification() {
    deleting.value = true;
    deleteNotificationConfirm.value = false;
    errorMessage.value = '';
    try {
