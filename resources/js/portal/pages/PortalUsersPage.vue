    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Manage credentials and authentication access for portal staff.
        </p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
            ...
            <tr v-for="u in users" :key="u.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all">
              <td class="px-6 py-4 font-semibold text-slate-900 dark:text-white">
                <div class="flex items-center gap-2">
                  {{ u.name }}
                  <span v-if="u.id === currentUserId" class="px-1.5 py-0.5 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded text-[10px] font-bold">You</span>
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <span
                  ...
                  :class="u.is_active ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400'"
                >
                  <span class="w-1.5 h-1.5 rounded-full" :class="u.is_active ? 'bg-emerald-500' : 'bg-slate-400'" />
                  {{ u.is_active ? 'Active' : 'Inactive' }}
                </span>
              </td>
              <td class="px-6 py-4 text-right space-x-2">
                <button class="text-xs font-semibold px-2.5 py-1.5 border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 rounded-lg transition-all cursor-pointer" @click="openEditModal(u)">
                  Edit
                </button>
                <button v-if="u.id !== currentUserId" class="text-xs font-semibold px-2.5 py-1.5 text-rose-600 dark:text-rose-400 border border-transparent hover:border-rose-500/35 hover:bg-rose-50/20 rounded-lg transition-all cursor-pointer" @click="promptDelete(u)">
                  Delete
                </button>
              </td>
            </tr>
    ...
    <div v-if="modal.show" class="fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-6 relative">
        <h3 class="text-lg font-bold mb-4">
          {{ modal.mode === 'create' ? 'Add Portal Administrator' : 'Edit Administrator Details' }}
        </h3>
        ...
        <form class="space-y-4" @submit.prevent="submitForm">
          ...
          <div>
            ...
            <input
              ...
              type="email"
              required
              placeholder="e.g. hashan@company.com"
              class="w-full px-3.5 py-2 bg-slate-100/50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 focus:border-indigo-500 rounded-lg text-sm outline-none transition-all"
            />
          </div>

          <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Mobile Number (For OTP Verification)</label>
            <input
          ...
          <div v-if="modal.userId !== currentUserId" class="flex items-center gap-2 py-2">
            <input
              id="is_active"
            ...
            />
            <label for="is_active" class="text-sm font-semibold text-slate-700 dark:text-slate-300">Grant Access (Active Account)</label>
          </div>
...
export default {
  setup() {
    ...
    const context = window.portalContext || { user: {} };
    const currentUserId = ref(context.user?.id || 0);
      ...
      otpModal.onVerifySuccess = async (otpCode) => {
        try {
          if (modal.mode === 'create') {
            ...
            await apiRequest(`/users/${modal.userId}`, {
              method: 'put',
              headers: { 'X-Portal-OTP': otpCode },
              data: {
                name: form.name,
          ...
          const errorMsg = err.response?.data?.message || 'Failed to save administrator details.';
    ...
    const promptDelete = (user) => {
      ...
      otpModal.onVerifySuccess = async (otpCode) => {
        if (!confirm(`Are you absolutely sure you want to permanently delete administrator account "${user.name}"?`)) {
          otpModal.show = false;
          return;
        }
        ...
        try {
          ...
          otpModal.error = err.response?.data?.message || 'Failed to delete administrator.';
        }
      };
    };
