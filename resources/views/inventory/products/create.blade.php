<x-app-layout>
    <x-slot name="title">Add Product - {{ app('tenant')->name }}</x-slot>
                    ...
                    <div class="bg-white dark:bg-secondary-900 rounded-xl shadow-sm border border-secondary-200 dark:border-secondary-700 p-6">
                        <form action="{{ route('inventory.products.store') }}" method="POST">
                            ...
                            <div class="flex items-center justify-end space-x-4 pt-6">
                                ...
                                <button type="submit" class="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-colors">Create Product</button>
                            </div>
                        </form>
                    </div>
                </div>
