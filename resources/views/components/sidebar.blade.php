...
<!-- Sidebar -->
<aside id="sidebar" data-collapsed="false" class="app-surface app-sidebar-panel fixed lg:static inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 lg:data-[collapsed=true]:-translate-x-full w-64 lg:data-[collapsed=true]:w-0 lg:data-[collapsed=true]:border-r-0 lg:data-[collapsed=true]:overflow-hidden border-y-0 border-l-0 rounded-none flex flex-col z-50 transition-[transform,width] duration-300 ease-in-out">
    <!-- Logo/Brand -->
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ...
        @if(auth()->user()->hasPermission('sales.process'))
        <a href="{{ route('sales.create') }}" data-collapse-sidebar="true" class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('sales.*') ? 'text-white bg-gradient-to-r from-primary-500 to-primary-700' : 'text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-700' }} rounded-lg transition-colors">
            <svg class="h-5 w-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
